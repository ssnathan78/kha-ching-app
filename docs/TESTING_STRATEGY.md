...
Strategy, risk, Chase, and ledger-provenance changes must add or extend **named** `lib/simulation/catalog.ts` scenarios that would fail if a phantom book, paper/live mix-up, or flatten-with-lots-on-a-flat-book came back. A 9:20 leftover wing after one SL is **not** a phantom book — add/keep `*-920-one-way-holds-other-leg`. Agents: follow **Adversarial testing** in [AGENTS.md](../AGENTS.md). Suites: `__tests__/simulation/chaseAdversarial.test.ts`, `__tests__/simulation/strategyAdversarial.test.ts`, `__tests__/simulation/straddle920.test.ts`, CORE list in `__tests__/simulation/scenarios.test.ts`. Run `yarn sim-test`. Every strategy (Chase, ATM straddle, ATM strangle) needs reject-without-fill and paper↔live coverage.
