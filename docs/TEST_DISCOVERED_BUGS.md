...
|----|----------|---------|-------------|--------|-------------------|
| BUG-001 | Medium | Exit strategies | UI listed unimplemented exit enums; queue only ran `INDIVIDUAL_LEG_SLM_1X` | **Fixed** — schedule-time validation + UI limited to allowed exits | `strategyValidation.test.ts`, `processExitJob.test.ts` |
...
| Area | Change |
|------|--------|
| BUG-001 | `lib/strategyValidation.ts` rejects unimplemented exits; `processExitJob.ts` throws on unsupported enum; UI uses `EXIT_STRATEGIES_ALLOWED_AT_SCHEDULE` |
