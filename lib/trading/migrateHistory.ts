...
import { applyUnappliedFills, ensureDefaultAccount } from "./ledger"

/**
 * Idempotent. SQL migration 0005 copies transactions → orders/fills.
 * This function reapplies that copy for rows inserted after migrate, then
 * books any fill that has not yet updated a position.
 */
export async function backfillFromTransactions(): Promise<{
  ...
  try {
    ...
    await pool.query(`
      ...
      SELECT
        o.account_id, o.id, o.job_id, o.strategy, o.broker_order_id, o.exchange, o.tradingsymbol,
        o.instrument_token, o.product, o.side, o.filled_qty, COALESCE(o.average_fill_price, 0),
        'migrated-txn:' || o.broker_order_id, COALESCE(o.filled_at, o.created_at), o.filled_at, 'MIGRATED'
      FROM orders o
      WHERE o.provenance = 'MIGRATED' AND o.broker_order_id IS NOT NULL
      ON CONFLICT (fingerprint) DO NOTHING
    `)
  } catch (e) {
    logger.error("[migrateHistory.backfillFromTransactions]", e)
  }

  const appliedFills = await applyUnappliedFills()
  void db
  return { insertedOrders, appliedFills }
}
